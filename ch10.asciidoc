...
// AU: in the following `include`, I changed the tag `data` to `dataset`. Is this correct?
[[torch_data]]
...
The RayPlugin class provides Distributed Data-Parallel (DDP) training on a Ray cluster. PyTorch DDP is used as the distributed training protocol by PyTorch, and Ray is used in this case to launch and manage the training worker processes. The base code using this plugin is shown in <<ltorch_train>>.footnote:[The full code is in https://github.com/scalingpythonml/scalingpythonml/blob/master/ray_examples/ml/torch/wine-quality_ltorch_ray.py[+++github+++].]
...
With these three functions in place, integration with Tune is very straightforward (<<tune_additions>>):
