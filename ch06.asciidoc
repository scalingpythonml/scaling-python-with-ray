...
To link this implementation to the Kafka consumer, we do need to modify the Kafka consumer (<<Kafka-consumer-actor>>) a little bit (<<integrating-kafka-consumer-with-temperature-controller-manager>>).
...
With these changes in place, you can implement the main program, similar to (<<bringing-it-all-together>>)footnote:[You can find the complete code for this implementation https://github.com/scalingpythonml/scalingpythonml/blob/master/streaming/persistent_ray/keyed/keyed_ray_kafka_persistent.py[+++here+++]] and start an execution. The partial execution result (in <<controller-execution-results>>) shows the output of processing:
