...
==== Installing (for x86 and Mac ARM)
...
For those running on systems without pre-built wheels, installing from source is possible but a little slow. See <<appendix_deploying>>.
