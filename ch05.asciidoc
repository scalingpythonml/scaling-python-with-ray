...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=flaky_remote_fun]
----
====
...
If your application fails, nearly all of the resources your application was using will eventually be garbage collected. The one exception is detached resources,footnote:[ Like detached actors and detached placement groups.] which Ray will restart as configured beyond the life of your current program provided the cluster does not fail.footnote:[This can prevent your cluster from scaling down as Ray will not release the resources.]
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=ex_ray_immuteable]
----
====
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=ex_use_ray_put]
----
====
...
By default, Ray will raise an ObjectLostError when you attempt to get a lost object. You can enable recomputing by providing pass:[<em>_enable_object_reconstruction=True</em>] to ray.init or adding pass:[<em>--enable-object-reconstruction</em>] to ray start. This recomputationfootnote:[This recomputation is done using information in the global control store.] will only happen once the object is needed, e.g. it is lazy on resolution.
...
Ray's object store uses reference counting garbage collectionfootnote:[The same algorithm as Python.] to cleanup objects which your program doesn't need anymore. The object store keeps track of both direct references, and ObjectRefs which refer to other ObjectRefs.footnote:[Same cycle problem as Python.]
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=ray_obj_store_config]
----
====
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=custom_serializer]
----
====
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=custom_serializer_not_own_class]
----
====
...
The ray.remote decorator takes num_cpus, num_gpus, and memory as parameters to indicate the amount of resources an actor or remote function will consume. The defaults are one CPU and zero GPUs. You can override this value by specifying required resources in the _@ray.remote_ annotation, for example, usage of annotation pass:[<em>@ray.remote(num_cpus=4, num_gpus=2)</em>] will request four CPUs and two GPUs for function execution. 
...
If you know how much memory a task or actor requires, you can specify it in the resource requirements of its ray.remote annotation to enable memory-aware scheduling.footnote:[Specifying a memory requirement does NOT impose any limits on memory usage. The requirements are used for admission control during scheduling only (similar to how CPU scheduling works in Ray). It is up to the task itself to not use more memory than it requested.] For example, `@ray.remote(memory=500 * 1024 * 1024)` will request 500MiB of memory to for this task.footnote:[Note that Ray itself does not impose any limits on the memory usage. It is up to the task itself to not use more memory than it requested.]  
...
The auto-scaler creates new nodes in response to the following events:footnote:[For more information on the worker nodes creation for different platforms refer to this https://docs.ray.io/en/master/cluster/cloud.html#cluster-cloud[documentation].]
...
* SDK pass:[<a href="https://docs.ray.io/en/latest/cluster/sdk.html"><em>request_resources</em></a>] function call is similar to the cluster creation request, but these resources are never released for the life of the cluster.
...
* _Creation:_ placement group creation request is sent to the Global Control Structure (GCS) that calculates how to distribute the bundles and sends resource reservation requests to all the nodes. Ray guarantees placement groups are placed _atomically_.
* _Allocation:_ placement groups are pending creation. If existing Ray nodes can satisfy resource requirements for a given strategy, then placing groups are allocated and success is returned. Otherwise, the result depends on whether Ray is able to add nodes. If the autoscaler is not present or the node limit is reached, then placement group allocation fails and the error is returned, otherwise the autoscaler scales the cluster to ensure pending groups can be allocated.
* _Node’s failure:_ when worker nodesfootnote:[Ray’s cluster head node is a single point of failure, so if it fails, the whole cluster will fail, as mentioned in <<fault-tolerance>>.] that contain some bundles of a placement group die, all the bundles will be rescheduled on different nodes by GCS. This means that the placement group creation _atomicity_ only applies to initial placement creation. Once a placement group is created, it can become partial due to node failures.
* _Cleanup:_ Ray automatically removes placement groups when the job that created the placement group is finished. If you’d like to keep the placement group alive regardless of the job that created it, you should specify lifetime=”detached” during placement group creation. You can also explicitly free a placement group at any time by calling pass:[<em>remove_placement_group</em>].
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=placement_group_imports]
----
====
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=placement_group]
----
====
...
.Mixed CPU & GPU Placement group
[[mixed_placement_group]]
====
[source, python]
----
include::examples/ray/details/Ray-Concepts-Remote.py[tags=mixed_placement_group]
----
====
...
You can assign placement groups names. You can achieve this by specifying a parameter pass:[<em>name="desired_name"</em>] at the point of placement group creation. This allows you to retrieve and use the placement group from any job in the Ray cluster by name rather than passing a placement group handle.
=== Namespaces
...
The fastest way to add a "few" packages to your runtime context is by specifying a list of packages from the https://pypi.org/[Python Package Index (PyPI)] you need. Looking at the web scraping example from chapter 2 where you used the beautiful soup library, you can ensure this package is available in a distributed environment by creating an execution context with it as shown in <<pip_pkg_list>>.
...
[source, python]
----
include::examples/ray/details/Ray-Concepts.py[tags=pip_pkg_reqs]
----
====

This works well for a few dependencies, but if you've got a requirements.txt like Holden's link:$$https://github.com/holdenk/print-the-world$$[print the world] project, you can also just point this to your local `requirements.txt` as shown in <<pip_pkg_reqs>>.
...
----
=== Conclusion
In this chapter, you've gained a deeper understanding of how Ray works. Your knowledge of serialization will help you understand which work to distribute and which to keep in the same process. You now know your different options and how to choose the right scaling technique. You have a few different techniques to manage Python dependencies, even conflicting ones, on your Ray cluster. You are well set up to learn about the higher-level building blocks covered in the next part of the book.
