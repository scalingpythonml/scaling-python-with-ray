...
Remote actors are a complicated case for fault tolerance as they contain state within them. In chapter <<ch04>>, this is why you explored the different options for persisting & recovering that state. Actors can experience failure at any stage: setup, message processing, or between messages.
...
The fastest way to add a "few" packages to your runtime context is by specifying a list of packages from https://pypi.org/[+++the Python Package Index (PyPI)+++] you need. Looking at the web scraping example from chapter 2 where you used the beautiful soup library, you can ensure this package is available in a distributed environment by creating an execution context with it as shown in <<pip_pkg_list>>.
