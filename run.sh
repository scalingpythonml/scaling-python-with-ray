#!/bin/bash
source .settings.sh
pod_name=$(kubectl get pod -n ray  -l ray-cluster-name=spacebeaver-cluster,ray-node-type=head -o name)
...
export RAY_HEAD_ADDRESS="http://localhost:10001"
...
#ray submit --screen --runtime-env-json'{"working_dir": "./", "pip": 
...
# Run "classic"
python ./messaging/main.py --head ${RAY_HEAD_ADDRESS}
