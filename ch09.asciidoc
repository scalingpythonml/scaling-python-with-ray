...
Ray Datasets do not have a built-in way to specify additional libraries to be installed. You can use `map_batches` along with a task to accomplish this, as shown in <<more_awesome_wordcount_with_batchest>> where we install extra libraries to parse the HTML.
