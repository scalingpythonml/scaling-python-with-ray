...
Ray offers two types of handles - synchronous and asynchronous. _Sync_ flag - `__Deployment.get_handle(..., sync=True|False)`__ can be used to specify a handle type.
...
You can request specific resource requirements in _`_@serve.deployment`. For example, two cpus and 1 GPU would be:
