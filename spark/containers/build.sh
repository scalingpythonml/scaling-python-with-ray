...
set -eux
#tag::dlspark[]
if [ ! -f spark-3.2.1-bin-hadoop3.2.tgz ]; then
  axel https://ftp.wayne.edu/apache/spark/spark-3.2.1/spark-3.2.1-bin-hadoop3.2.tgz
fi
rm -rf spark-3.2.1-bin-hadoop-3.2
...
SPARK_VERSION=3.2.1.1
cp pysetup.sh ${SPARK_HOME}/bin/
pushd ${SPARK_HOME}
SPARK_ROOT="$SPARK_HOME"
./bin/docker-image-tool.sh  -r holdenk -t v${SPARK_VERSION} -X -b java_image_tag=11-jre -p PyDockerfile Dockerfile build
#end::build_exec_containers[]
#tag::build-notebook[]
docker buildx build -t holdenk/spark-notebook:v${SPARK_VERSION}  --platform linux/arm64,linux/amd64 --build-arg base_img=holdenk/spark-py:v${SPARK_VERSION} --push .
#end::build-notebook[]
popd
