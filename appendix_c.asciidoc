...
// AU: note markup was empty in the previous file of record. add note?
[NOTE]
====
====
...
Visual Studio Code (VSC) uses the https://github.com/microsoft/debugpy/[+++debugpy+++] extension to expose debugging. It works similar to pdb in that it exposes a port for VS to connect to. The first step is to add the `debugpy` package to your container/requirements. Then, in the actor you want to debug, you can enable the debugger as shown in <<ex_vscode>>.

https://github.com/microsoft/debugpy/[+++TODOEX+++]

Then you will need to find the container that the actor is scheduled on and connect to it from VSC.
...
Exit codes are numeric codes that are set when a program exits, with any value besides 0 normally indicating failure. These codes (by convention) generally have meaning but are not 100% consistent. Some common exit codes are shown in <<table-c-01>>.
