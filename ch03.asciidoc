...
We'll further explore remote objects and their fault tolerance in <<ray_objects>>.
****

Some details in <<sleepy_task>> from the previous chapter are worth understanding. The example converts the iterator from a `iterator` to a `list` before passing it to `ray.get`. You need to do this when calling `ray.get` takes in a list of futures or an individual future.footnote:[Ray does not "go inside" classes or structures to resolve futures, so if you have a list of lists of futures or a class containing a future, Ray will not resolve the "inner" future.] `ray.get` waits until it has all of the objects so that it can return the list in order.
...
[source, python]
----
include::examples/ray/remote/Ray-Remote.ipynb[tags=get_only]
----
====
...
[source, python]
----
include::examples/ray/remote/Ray-Remote.ipynb[tags=as_available]
----
====
...
[source, python]
----
include::examples/ray/remote/Ray-Remote.ipynb[tags=handle_bad_futures]
----
====
...
[source, python]
----
include::examples/ray/remote/Ray-Remote.ipynb[tags=ray_remote_seq]
----
====
...
You _cannot_ use Python structures (for example, lists, dictionaries, or classes) containing `ObjectRef` instead of using `ObjectRef` directly. Ray only waits for and resolves pass:[<code>ObjectRef</code>s] that are passed directly to a function. If you attempt to pass a structure, you will have to do your own `ray.wait` + `ray.get` inside of the function. <<broken_ray_remote_seq>> shows a variation of code from <<ray_remote_seq>> that does not work.
...
[source, python]
----
include::examples/ray/remote/Ray-Remote.ipynb[tags=nested_par]
----
====

This code defines three different remote functions:

`generate_numbers`:: A simple function that generates random numbers
...
Machine learning models composed of other models, e.g., ensemble models, are well suited to evaluation with Ray. <<ensemble_example>> shows what it looks like to use Ray's function composition for a hypothetical spam model for web links.
