...
If we take the basic code used for the creation of the model in <<ch07>>, and try to optimize parameters for the decision tree, our code will look as in <<skex>>:
...
Ray https://docs.ray.io/en/master/ray-more-libs/joblib.html[+++implements+++] a backend for joblib with Ray actors pool (see <<ch04>>) instead of local processes. This allows you to simply change the joblib backend to switch scikit learn from using local processes to Ray.
