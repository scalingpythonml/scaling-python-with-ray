...
[TIP]
====
As covered in <<ray_objects>>, Ray Datasets default behaviour may be different than you expect. You can enable object recovery by setting `enable_object_reconstruction=True` in `ray.init` to make Ray Datasets more resilient.
====
...
* Support for data structures including pick-listsfootnote:[Like an enum], hash tables and queues.
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=load_csv_local_fs]
----
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=load_from_https]
----
====

[WARNING]
====
At present, the protocol is incorrectly stripped off, so you need to put it in twice, for example when loading data from an https website you would load from `about:blank[+++https://https://[someurlhere].com+++]`.
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=wc_write]
----
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=enable_dask]
----
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=enable_spark]
----
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=to_dask]
----
====
...
You are not limited to the tools that are built into Ray. If you have a new tool that supports Apache Arrow, and https://arrow.apache.org/docs/python/api/datatypes.html[+++you are using Apache Arrow supported types+++], `to_arrow_refs()` gives you a zero copy Arrow representation of your dataset. You can then use this list of Ray Arrow objects to pass into your tool, be it model training or any other purpose. You will learn more about this in the "Built-in Ray DataSet operations" section.
...
* https://github.com/modin-project/modin[Modin]
...
This section assumes you have a good understanding of the data wrangling tools you're going to use with ray, either pandas or Apache Spark. Pandas is ideal for users scaling Python analytics, and Spark is well suited for users connecting to big data tools. If you're not familiar with the Pandas APIs, you should check out _Python for Data Analysis: Data Wrangling with Pandas, NumPy, and IPython_ by Wes McKinney. New Spark users should check out _Learning Spark_, and if you want to go super deep Holden recommends _High Performance Spark_ by Holden and Rachelfootnote:[Note: this is like a ford dealer recommending a ford, so take the advice here with a grain of salt.].
...
https://dask.org/[+++Dask+++] on Ray is an excellent choice for data preparation for machine learning, or scaling existing pandas code. Many initial Dask developers also worked on Pandas leading to a comparatively solid distributed pandas interface.

[NOTE]
====
Portions of this section are based on the DataFrame chapter in _Scaling Python with Dask_ (O'Reilly).
====
...
Slow/inefficient aggregationsfootnote:[Or very likely to cause an out of memory exception.] use storage proportional to the records being aggregated. Examples from this slow group include making a list, and naively computing exact quantilesfootnote:[There are alternate algorithms for exact quantiles that depend on more shuffles to reduce the space overhead.]. With these slow aggregates, there is no benefit to using Dask's aggregation class over the `apply` API which you may wish to use for simplicity. For example, if you just wanted a list of employer IDs by postal code, rather than having to write three different functions you could use a one-liner like `df.groupby("PostCode")["EmployerId"].apply(lambda g: list(g))`. Apply is implemented as a "full-shuffle" which is covered in the next section.
...
[WARNING]
====
Dask, including rolling time windows, assumes that your partition index is monotonically increasing.footnote:[Strictly increasing with no repeated values (e.g. 1,4,7 is monotonically increasing but 1,4,4,7 is not).]
====
...
[TIP]
====
If you are interested in going deeper with Dask there are several Dask focused books in active development, including _Scaling Python with Dask_ by Holden Karau and Mika Kimmins where much of this data frame chapter is based from.
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=ray_wordcount_on_ds_filter_only_once_with_batches]
----
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=batch_op_on_pandas]
----
====
...
Ray Datasets do not have a built-in way to specify additional libraries to be installed. You can use `map_batches` along with a task to accomplish this, as shown in <<more_awesome_wordcount_with_batches>> where we install extra libraries to parse the HTML.
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=more_awesome_wordcount_with_batches]
----
====
...
[source, python]
----
include::examples/ray_examples/data/Ray-DataSets.py[tags=agg]
----
====
